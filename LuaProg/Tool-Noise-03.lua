...
x, y = M.hist(A, 64)
hist = P("HIST", filename .. " HIST(STD(PHASE))", x, V.norm(y), "Histogram" )
chart.set_xlabel(hist, "STD(PHASE)")
chart.set_ylabel(hist, "Relative Ocurrence")

-------------------------------------------
p = surface("IMAGE", filename .. " STD(PHASE)")
...
surface.set_title1(p, "Standard Deviation of the Phase")
surface.set_title2(p, data_tape.get_filename(tape))
surface.set_xlabel(p, "Frequency (MHz)")
surface.set_ylabel(p, "STD(Phase)")
surface.update(p)
