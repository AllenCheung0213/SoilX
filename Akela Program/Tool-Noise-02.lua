...
X2 = M(NF, NS)
for i = 1, NF do
	X1[i] = V.cumsum(CV.abs(T[i]))
	X2[i] = V.cumsum(CV.abs(T[i])^2)
end
...
x, y = M.hist(A, 64)
hist = plot("HIST", filename .. " HIST(STD(MAG))", x, V.norm(y), "Histogram" )
chart.set_xlabel(hist, "STD(MAG)")
chart.set_ylabel(hist, "Relative Ocurrence")

-------------------------------------------
p = surface("IMAGE", filename .. " STD(MAG)")
...
surface.set_title1(p, "Standard Deviation of the Magnitude")
surface.set_title2(p, data_tape.get_filename(tape))
surface.set_xlabel(p, "Frequency (MHz)")
surface.set_ylabel(p, "STD(MAG)")
surface.update(p)
